...
export default function NewCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const newCategory = {
        ...
        name,
        description,
        image: image || 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?q=80&w=1000&auto=format&fit=crop',
    };
    ...
    try {
        ...
        alert('Failed to save category. Even with our new optimized storage, something went wrong. Please try refreshing or using a different image URL.');
        setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-10 pb-20">
      <div className="flex items-center gap-4">
        ...
        <div>
           <h1 className="text-4xl font-black uppercase tracking-tighter">Create New Category</h1>
           <p className="text-zinc-500 font-medium tracking-tight">Design a new collection for your store.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="space-y-8">
            <div className="bg-zinc-900 text-white p-8 rounded-[32px] shadow-2xl relative overflow-hidden">
                ...
                <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-tight mt-4 relative z-10">
                    Creating this category will make it immediately available for product assignment.
                </p>
            </div>
            ...
            <div className="bg-white p-8 rounded-[32px] border border-zinc-200 shadow-sm space-y-4">
                ...
                >
                    {isLoading ? <div className="w-6 h-6 border-4 border-white/20 border-t-white rounded-full animate-spin" /> : <><Save className="w-5 h-5" /> SAVE CATEGORY</>}
                </button>
                <button 
                  type="button"
                  onClick={() => router.back()}
